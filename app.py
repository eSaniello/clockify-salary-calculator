    ...
    with open("data.csv", mode="r") as csv_file:
        ...
        for row in csv_reader:
            ...
            if row["User"].lower().strip() == name.lower().strip():
                print(
                    f'Name: {row["User"]}, Email: {row["Email"]}, Date: {row["End Date"]} Duration (h): {row["Duration (h)"]}, Duration (decimal): {row["Duration (decimal)"]}, Billable Rate (USD): {row["Billable Rate (USD)"]}, Billable Amount (USD): {row["Billable Amount (USD)"]}'
                )

                todays_billable = float(row["Duration (decimal)"]) * float(rate)
                ...
                billable_amount += float(row["Duration (decimal)"]) * float(rate)
        ...
        print("Billable amount extra (weekends) (USD):", billable_amount_extra)
        print("=====================================")


# call function
input_name = input(
    "Please enter the name of the employee. (Make sure to use name in Clockify) \n"
)
employee_rate = input("Please enter the hourly rate of this employee \n")
calcPay(input_name, employee_rate)
